...
class PSA_STree(object):
    ...
    def _dfs_leaves(self, node :Node, results :list, length :int):
        ...
        for son in sons:
            if son.leaf:
                results.append(son.star t -lengt h -self._edge_length(node))
            else:
    ...
    def _select_prefix(self, s :str):
        ...
        while node.children.get(s[ste p +length]) is not None:
            node = node.children[s[ste p +length]]
            step += 1
            ...
            if self.T[node.start + step] != s[ste p +length]:
                break

            while self.T[node.start + step] == s[ste p +length]:
                step += 1
                if len(s) >= step + length:
                    if self.__walk_down_fcs(node, step):
                        length += step
            ...
            if self.T[node.start + step] != s[ste p +length]:
                break
        if node.leaf:
            starts.append(node.star t -length)
